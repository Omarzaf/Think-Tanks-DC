...
export function TreemapPanel({ tanks }: { tanks: ThinkTank[] }) {
  ...
  const hierarchy = useMemo(() => {
    ...
    return {
      ...
      children: Object.values(groups).map(g => ({
        name: g.name, ideology: g.ideology,
  ...
  const draw = useCallback(() => {
    ...
    const root = d3.hierarchy(hierarchy).sum((d: any) => d.value || 0).sort((a, b) => (b.value || 0) - (a.value || 0));
    d3.treemap<any>().size([width, height]).padding(3).paddingTop(20).round(true)(root);

    const groups = svg.selectAll('.group').data(root.children || []).join('g').attr('class', 'group');
    groups.append('rect')
      .attr('x', (d: any) => d.x0).attr('y', (d: any) => d.y0)
      .attr('width', (d: any) => d.x1 - d.x0).attr('height', (d: any) => d.y1 - d.y0)
      .attr('fill', 'none').attr('stroke', (d: any) => IDEOLOGY_COLORS[d.data.ideology as Ideology] || '#666').attr('stroke-width', 1.5).attr('rx', 6);
    groups.append('text')
      .attr('x', (d: any) => d.x0 + 5).attr('y', (d: any) => d.y0 + 14)
      .attr('fill', (d: any) => IDEOLOGY_COLORS[d.data.ideology as Ideology] || '#666').attr('font-size', 10).attr('font-weight', 700)
      .text((d: any) => { const w = d.x1 - d.x0; const label = `${d.data.name} (${d.children?.length || 0})`; return w > 80 ? label : (w > 40 ? d.data.name : ''); });
    ...
    leaves.append('text')
      .attr('x', (d: any) => d.x0 + 4).attr('y', (d: any) => d.y0 + 13).attr('fill', '#1a1a1a')
      .attr('font-size', (d: any) => { const w = d.x1 - d.x0; const h = d.y1 - d.y0; if (w < 45 || h < 18) return 0; return w > 100 ? 9 : 7; })
    ...
    <div style={{ display: 'flex', gap: 8, fontSize: 9, alignItems: 'center' }}>
      {(Object.entries(IDEOLOGY_COLORS) as [Ideology, string][]).map(([label, color]) => (
        <span key={label} style={{ display: 'flex', alignItems: 'center', gap: 2, color }}>
          <span style={{ width: 8, height: 8, background: color, borderRadius: 2, display: 'inline-block', opacity: 0.7 }} />{label}
        </span>
      ))}
    </div>
  );
