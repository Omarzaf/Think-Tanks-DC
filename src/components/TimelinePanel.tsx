...
export function TimelinePanel({ tanks }: { tanks: ThinkTank[] }) {
  ...
  useEffect(() => {
    ...
    g.append('text')
      .attr('x', 0).attr('y', -12)
      .attr('fill', TEXT_COLOR).attr('font-size', 11).attr('font-weight', 600)
      .text('Founding Years of 75 Think Tanks');

    // Decade grid lines
    for (let decade = Math.ceil(minYear / 10) * 10; decade <= maxYear; decade += 10) {
      g.append('line')
        .attr('x1', xScale(decade)).attr('x2', xScale(decade))
        .attr('y1', 0).attr('y2', timelineH)
        .attr('stroke', GRID_COLOR).attr('stroke-width', 0.5);
      g.append('text')
        .attr('x', xScale(decade)).attr('y', timelineH + 14)
        .attr('text-anchor', 'middle')
        .attr('fill', TEXT_MUTED).attr('font-size', 9)
        .text(decade.toString());
    }

    // Jitter tanks vertically
    ...
    sorted.forEach(tank => {
      ...
      if (slots.length === 0) {
        ...
        const offset = (Math.ceil(slots.length / 2)) * 14 * (slots.length % 2 === 0 ? 1 : -1);
        y = baseY + offset;
      }
      y = Math.max(8, Math.min(timelineH - 8, y));
      slots.push(y);
      usedSlots.set(x, slots);
      yPositions.push(y);
    });
    ...
    sorted.forEach((tank, i) => {
      ...
      g.append('circle')
        .attr('cx', cx)
        .attr('cy', cy)
        .attr('r', tank.isDarkMoney ? 3.5 : Math.max(3.5, Math.min(7, Math.log10(tank.totalFunding + 1) * 0.8)))
        .attr('fill', color)
        .attr('opacity', tank.isDarkMoney ? 0.4 : 0.75)
        .attr('stroke', tank.isDarkMoney ? '#d1d5db' : '#ffffff')
        .attr('stroke-width', 1)
        ...
        .on('mouseout', function () {
          ...
          d3.select(this).attr('r', r).attr('opacity', tank.isDarkMoney ? 0.4 : 0.75).attr('stroke-width', 1);
          hide();
        });
    });
    ...
    decades.forEach(([decade, counts]) => {
      ...
      const fundedH = totalH - darkH;

      barG.append('rect')
        .attr('x', x).attr('y', barYScale(counts.funded + counts.dark))
        .attr('width', barWidth).attr('height', Math.max(0, fundedH))
        .attr('fill', '#4338ca').attr('rx', 3).attr('opacity', 0.8);

      barG.append('rect')
        .attr('x', x).attr('y', barYScale(counts.dark))
        .attr('width', barWidth).attr('height', Math.max(0, darkH))
        .attr('fill', DARK_MONEY_COLOR).attr('rx', 3).attr('opacity', 0.9)
  ...
  return (
    <PanelWrapper
      title="Timeline"
      subtitle="Founding years + establishment trends by decade"
      legend={
        <div style={{ display: 'flex', gap: 10, fontSize: 9, color: TEXT_MUTED }}>
          <span><span style={{ color: '#4338ca' }}>●</span> Disclosed</span>
          <span><span style={{ color: '#9ca3af' }}>●</span> Dark Money</span>
        </div>
      }
    >
