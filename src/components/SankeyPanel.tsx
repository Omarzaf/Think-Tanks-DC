...
export function SankeyPanel({ transactions }: { transactions: Transaction[] }) {
  ...
  const draw = useCallback(() => {
    ...
    try {
      const filtered = filter === 'All' ? transactions : transactions.filter(t => t.donorType === filter);
      const { nodes, links } = getSankeyData(filtered, 15);
      if (nodes.length === 0 || links.length === 0) return;

      const margin = { top: 10, right: 160, bottom: 10, left: 160 };
      const nodeList = nodes.map(n => ({ ...n, name: n.id.replace(/^(donor|tank):/, '') }));
      const nodeIndexMap = new Map(nodeList.map((n, i) => [n.id, i]));
      const linkList = links
        .filter(l => nodeIndexMap.has(l.source) && nodeIndexMap.has(l.target))
        .map(l => ({ source: nodeIndexMap.get(l.source)!, target: nodeIndexMap.get(l.target)!, value: l.value, donorType: l.donorType }));
      ...
      const graph = sankeyLayout({ nodes: nodeList.map(d => ({ ...d })), links: linkList.map(d => ({ ...d })) });
      const g = svg.append('g');

      g.selectAll('.link').data(graph.links).join('path')
        .attr('class', 'link').attr('d', sankeyLinkHorizontal() as any)
        .attr('fill', 'none')
        .attr('stroke', (d: any) => DONOR_TYPE_COLORS[d.donorType as DonorType] || '#666')
        .attr('stroke-opacity', 0.35).attr('stroke-width', (d: any) => Math.max(1, d.width))
        .style('cursor', 'pointer')
        .on('mouseover', function (event: MouseEvent, d: any) {
          d3.select(this).attr('stroke-opacity', 0.7);
          show(event, (
            <div>
              <div style={{ fontWeight: 600 }}>{(d.source.name || '').replace(/^(donor|tank):/, '')}</div>
              <div>→ {(d.target.name || '').replace(/^(donor|tank):/, '')}</div>
        ...
        .on('mouseout', function () { d3.select(this).attr('stroke-opacity', 0.35); hide(); });

      g.selectAll('.node').data(graph.nodes).join('rect')
        .attr('class', 'node')
        .attr('x', (d: any) => d.x0).attr('y', (d: any) => d.y0)
        .attr('width', (d: any) => d.x1 - d.x0).attr('height', (d: any) => Math.max(1, d.y1 - d.y0))
        .attr('fill', (d: any) => d.type === 'tank' ? '#475569' : DONOR_TYPE_COLORS[d.donorType as DonorType] || '#666')
      ...
      g.selectAll('.label').data(graph.nodes).join('text')
        .attr('class', 'label')
        ...
        .attr('fill', TEXT_COLOR).attr('font-size', 8)
        .text((d: any) => { const name = (d.name || '').replace(/^(donor|tank):/, ''); return name.length > 25 ? name.substring(0, 24) + '…' : name; });
    } catch (e) {
  ...
  const legend = (
    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
      {FILTER_OPTIONS.map(opt => (
        <button key={opt} onClick={() => setFilter(opt)} style={{ background: filter === opt ? '#f3f4f6' : 'transparent', border: '1px solid #d1d5db', borderRadius: 6, color: opt === 'All' ? TEXT_MUTED : DONOR_TYPE_COLORS[opt as DonorType], fontWeight: filter === opt ? 600 : 400, fontSize: 9, padding: '2px 6px', cursor: 'pointer' }}>
